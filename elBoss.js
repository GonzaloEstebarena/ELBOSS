const elBoss = {
   ...
   const element = elBoss.poderes[i];
       console.log(`tu ${i+1}º poder es  ${element.nombre} y hace ${element.description}`)
   
      }

     
    console.log('le vamos a decir "También se tu punto más debil, tus familiares, y les conozco...')
   ...
   const element = elBoss.reto[i];
   
   for (let i = 0; i < element[0].sigueElReto.length; i++) {
      const retaco = element[0].sigueElReto[i];
      
      let finalFrase =retaco.sigueElReto2.sigueElReto3.pareceQueSabesBucearEntreObjetos.definitivamenteSabes
      console.log(finalFrase[0],finalFrase[1],finalFrase[2],finalFrase[3],finalFrase[4],finalFrase[6],finalFrase[7])  
 ...
 for (let i = 0; i < elBoss.poderes.length; i++) {
  ...
  for (let i = 0; i < element.imagenes.length; i++) {
   const imPoderes = element.imagenes[i];
   console.log(`${imPoderes.imagen.url }`)
 ...
 const element = elBoss.familiares[i];
  ...
  edadTotal += element.edad;
  numero++
 }
 console.log('Media de la edad de los familiares: '+edadTotal/numero)
...
for (let i = 0; i < elBoss.reto.length; i++) {
       ...
       for (let i = 0; i < element[0].sigueElReto.length; i++) {
          ...
          let finalFrase =retaco.sigueElReto2.sigueElReto3.pareceQueSabesBucearEntreObjetos.definitivamenteSabes
          console.log(finalFrase[8].mensajeFinal)
